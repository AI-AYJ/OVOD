...
class YOLOv5WOTRBBoxTextGroundingDataset(BatchShapePolicyDataset, CocoDataset):
    ...
    def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
        ...
        texts = []
        for ann in ann_info:
            cat_name = ann.get("bbox_text", "").strip()
            if cat_name == "":
                continue
        ...
        for i, ann in enumerate(ann_info):
            ...
            cat_name = ann.get("bbox_text", "").strip()
            if cat_name == "" or cat_name not in cat2id:
                continue
        ...
        if len(texts) == 0 or len(instances) == 0:
            print("[EMPTY]", img_info["file_name"], "texts:", len(texts), "inst:", len(instances))
