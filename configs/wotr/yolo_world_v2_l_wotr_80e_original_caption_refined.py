# configs/yolo_world/wotr_openvocab_finetune.py
...
custom_imports = dict(
    imports=[
        'yolo_world',
        'yolo_world.datasets.wotr_bboxtext',
    ],
    allow_failed_imports=False
)

# -------------------------
# Hyper-parameters (open-vocab style)
# -------------------------
num_classes = 20
...
# (선택) stage1/사전학습 weight
load_from = '/home/heat/young/YOLO-World/pretrained_models/l_stage1-7d280586.pth'
...
# -------------------------
# WOTR dataset (bbox_text 기반, Flickr 포맷처럼 동작)
# -------------------------
wotr_train_dataset = dict(
    _delete_=True,
    type='YOLOv5WOTRBBoxTextGroundingDataset',   # ✅ 핵심
    data_root='/home/heat/young/WOTR_YOLO/',
    ann_file='annotations_caption/instances_train_refined_clean.json',  # ✅ bbox_text 포함
...
# -------------------------
# Val/Test: open-vocab 평가하려면 "LoadText"에 넣을 텍스트를 정의해야 함
# 지금은 우선 close-set처럼 WOTR 20 클래스로 평가할 수도 있고,
# open-vocab으로 하려면 평가 시 사용할 text list를 별도로 설계해야 함.
# 여기서는 일단 기존 WOTR COCO-format val/test를 그대로 둠.
# -------------------------
test_pipeline = [
...
# ⚠️ 여기 dataset은 "평가" 용이라 bbox_text 기반 dataset을 굳이 안 써도 됨
# (원하면 val/test도 bbox_text 기반으로 바꿀 수 있음)
wotr_val_dataset = dict(
    _delete_=True,
    type='MultiModalDataset',
    dataset=dict(
        type='YOLOv5CocoDataset', # ✅ 표준 검증용 클래스 (수정하지 마세요)
        data_root='/home/heat/young/WOTR_YOLO/',
        ann_file='annotations/instances_val.json', # ✅ 표준 정답지
        data_prefix=dict(img='images/val/'),
        test_mode=True,
        filter_cfg=dict(filter_empty_gt=False, min_size=32),
        metainfo=dict(classes=wotr_classes),
    ),
    # ✅ "모델아, 이 20개 단어를 찾아봐" (시험 문제지)
